'use client'
const GUTI_BUILD_V390_ADMIN='3.9.0'
...
export default function Page(){
 ...
 const [coupons,setCoupons]=useState([]),[daily,setDaily]=useState([]),[audit,setAudit]=useState([]),[reconciliation,setReconciliation]=useState([]),[reconBusy,setReconBusy]=useState(false),[opsAlerts,setOpsAlerts]=useState([]),[couponForm,setCouponForm]=useState({code:'',name:'',discount_type:'fixed',discount_value:'50',min_subtotal:'200',max_discount:'',merchant_id:'',first_order_only:false,max_uses:'',per_user_limit:'1',starts_at:'',ends_at:'',is_active:true})
 ...
 async function load(){
  const [o,f,m,c,s,d,cp,dy,au,rc,al]=await Promise.all([
  ...
  setOrders(o.data||[]);setFin(f.data||[]);setMerchants(m.data||[]);setCouriers(c.data||[]);setSettlements(s.data||[]);setCashDeposits(d.data||[]);setCoupons(cp.data||[]);setDaily(dy.data||[]);setAudit(au.data||[]);setReconciliation(rc.data||[]);setOpsAlerts(al.data||[])
 }
