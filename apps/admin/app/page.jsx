...
const merchantInitial={name:'',slug:'',merchant_type:'restaurant',description:'',phone:'',address:'',delivery_mode:'guti',commission_percent:'10',prep_minutes:'25',owner_name:'',owner_email:'',owner_phone:'',owner_password:'',open_time:'09:00',close_time:'21:00'}
const courierInitial={full_name:'',email:'',phone:'',password:'',vehicle_type:'Moto'}
...
export default function Page(){
 ...
 async function load(){
  const [o,f,m,c]=await Promise.all([
  ...
  setOrders(o.data||[]);setFin(f.data||[]);setMerchants(m.data||[]);setCouriers(c.data||[])
 }
 ...
 return <div className="layout"><aside><div className="logo">Guti<span>.mx</span></div><i>● Operación en vivo</i>{[['home','Resumen'],['orders','Pedidos'],['money','Finanzas'],['merchants','Negocios'],['couriers','Repartidores']].map(([a,b])=><button key={a} className={tab===a?'on':''} onClick={()=>setTab(a)}>{b}</button>)}<button onClick={()=>supabase.auth.signOut()}>Salir</button></aside><main>
  <header><div><small>GUTIÉRREZ ZAMORA</small><h1>{{home:'Centro de operación',orders:'Pedidos',money:'Finanzas',merchants:'Negocios',couriers:'Repartidores'}[tab]}</h1></div><div className="header-actions">{tab==='merchants'&&<button className="primary" onClick={()=>setShowMerchant(true)}>+ Crear negocio</button>}{tab==='couriers'&&<button className="primary" onClick={()=>setShowCourier(true)}>+ Registrar repartidor</button>}<button onClick={load}>↻ Actualizar</button></div></header>
