...
export default function Page(){
 ...
 const [coupons,setCoupons]=useState([]),[daily,setDaily]=useState([]),[audit,setAudit]=useState([]),[reconciliation,setReconciliation]=useState([]),[reconBusy,setReconBusy]=useState(false),[opsAlerts,setOpsAlerts]=useState([]),[central,setCentral]=useState([]),[incidents,setIncidents]=useState([]),[refunds,setRefunds]=useState([]),[couponForm,setCouponForm]=useState({code:'',name:'',discount_type:'fixed',discount_value:'50',min_subtotal:'200',max_discount:'',merchant_id:'',first_order_only:false,max_uses:'',per_user_limit:'1',starts_at:'',ends_at:'',is_active:true})

 useEffect(()=>{try{const x=localStorage.getItem('guti-admin-tab-v39');if(x)setTab(x)}catch{}},[])
 useEffect(()=>{try{localStorage.setItem('guti-admin-tab-v39',tab)}catch{}},[tab])
 ...
 useEffect(()=>{
  ...
  const nh=supabase.channel('admin-alerts-v39').on('postgres_changes',{event:'INSERT',schema:'public',table:'notifications'},p=>{if(p.new?.type==='stuck_order')load()}).subscribe()
  return()=>{s.subscription.unsubscribe();supabase.removeChannel(ch);supabase.removeChannel(nh)}
 },[])
 ...
 async function load(){
  const [o,f,m,c,s,d,cp,dy,au,rc,al,ce,inc,rf]=await Promise.all([
   ...
   supabase.from('coupons').select('*,merchants(name)').order('created_at',{ascending:false}),
   supabase.from('daily_operations_v38').select('*').limit(30),
   supabase.from('audit_logs').select('*').order('created_at',{ascending:false}).limit(150),
   supabase.from('payment_reconciliation_v39').select('*').order('created_at',{ascending:false}).limit(150),
   supabase.from('notifications').select('*').eq('type','stuck_order').order('created_at',{ascending:false}).limit(40),
   supabase.from('admin_central_operations_v40').select('*').order('age_minutes',{ascending:false}),
   supabase.from('order_incidents').select('*').order('created_at',{ascending:false}).limit(100),
   supabase.from('payment_refunds').select('*').order('requested_at',{ascending:false}).limit(100)
  ])
  if(o.error)return setMsg(o.error.message)
  setOrders(o.data||[]);setFin(f.data||[]);setMerchants(m.data||[]);setCouriers(c.data||[]);setSettlements(s.data||[]);setCashDeposits(d.data||[]);setCoupons(cp.data||[]);setDaily(dy.data||[]);setAudit(au.data||[]);setReconciliation(rc.data||[]);setOpsAlerts(al.data||[]);setCentral(ce.data||[]);setIncidents(inc.data||[]);setRefunds(rf.data||[])
 }
 ...
 async function dismissOpsAlert(id){
  await supabase.from('notifications').update({read_at:new Date().toISOString()}).eq('id',id);setOpsAlerts(x=>x.map(a=>a.id===id?{...a,read_at:new Date().toISOString()}:a))
 }

 async function refreshClipReconciliation(){
  setReconBusy(true);setMsg('')
  try{const data=await apiPost('/api/admin/reconcile-clip',{limit:80});setMsg(`Clip revisado: ${data.checked||0} pagos, ${data.mismatches||0} diferencias.`);await load()}catch(e){setMsg(e.message)}finally{setReconBusy(false)}
 }
 async function downloadSettlement(s){
  ...
  const rows=[['Pedido','Fecha','Negocio','Método','Total','Monto liquidado'],...(data||[]).map(x=>[x.orders?.id||'',new Date(x.orders?.created_at).toLocaleString('es-MX'),x.orders?.merchants?.name||'',x.orders?.payment_method||'',x.orders?.total||0,x.amount||0])]
 ...
 async function createCoupon(e){
  ...
  const payload={
    code:couponForm.code.trim().toUpperCase(),name:couponForm.name.trim(),discount_type:couponForm.discount_type,
    discount_value:Number(couponForm.discount_value||0),min_subtotal:Number(couponForm.min_subtotal||0),
    max_discount:couponForm.max_discount?Number(couponForm.max_discount):null,merchant_id:couponForm.merchant_id||null,
    first_order_only:couponForm.first_order_only,max_uses:couponForm.max_uses?Number(couponForm.max_uses):null,
    per_user_limit:Number(couponForm.per_user_limit||1),starts_at:couponForm.starts_at?new Date(couponForm.starts_at).toISOString():null,
    ends_at:couponForm.ends_at?new Date(couponForm.ends_at).toISOString():null,is_active:couponForm.is_active,created_by:session.user.id
  }
  if(!payload.code||!payload.name)return setMsg('Código y nombre son obligatorios.')
  const {error}=await supabase.from('coupons').insert(payload)
  if(error)return setMsg(error.message)
  setCouponForm({code:'',name:'',discount_type:'fixed',discount_value:'50',min_subtotal:'200',max_discount:'',merchant_id:'',first_order_only:false,max_uses:'',per_user_limit:'1',starts_at:'',ends_at:'',is_active:true})
  setMsg('Cupón creado.');await load()
 }
 ...
 async function refundClip(o){
  const reason=prompt(`Motivo del reembolso para #${o.id.slice(0,8)}`,'Cancelación autorizada por Guti')
  if(!reason)return
  if(!confirm(`Se solicitará a Clip un reembolso de ${money(o.total)}. ¿Continuar?`))return
  try{const x=await apiPost('/api/admin/refund-clip',{order_id:o.id,reason});setMsg('Reembolso aprobado por Clip.');setSelected(null);await load()}catch(e){setMsg(e.message)}
 }
 async function createIncident(o){
  const description=prompt(`Describe la incidencia del pedido #${o.id.slice(0,8)}`);if(!description)return
  const {error}=await supabase.from('order_incidents').insert({order_id:o.id,reported_by:session.user.id,reporter_role:'admin',category:'operacion',description,priority:'high'})
  if(error)return setMsg(error.message);setMsg('Incidencia registrada.');await load()
 }
 async function resolveIncident(i){const resolution=prompt('¿Cómo se resolvió?','Resuelto por Guti Admin');if(!resolution)return;const {error}=await supabase.from('order_incidents').update({status:'resolved',resolution,resolved_at:new Date().toISOString(),resolved_by:session.user.id}).eq('id',i.id);if(error)return setMsg(error.message);await load()}

 return <div className="layout"><aside><div className="logo">Guti<span>.mx</span></div><i>● Operación en vivo</i>{[['home','Central operativa'],['orders','Pedidos'],['money','Finanzas'],['reconciliation','Conciliación Clip'],['settlements','Liquidaciones'],['growth','Promociones'],['close','Cierre diario'],['audit','Auditoría'],['merchants','Negocios'],['couriers','Repartidores']].map(([a,b])=><button key={a} className={tab===a?'on':''} onClick={()=>setTab(a)}>{b}</button>)}<button onClick={()=>supabase.auth.signOut()}>Salir</button></aside><main>
  <header><div><small>GUTIÉRREZ ZAMORA</small><h1>{{home:'Central de operación',orders:'Pedidos',money:'Finanzas',reconciliation:'Conciliación de pagos',settlements:'Liquidaciones y efectivo',growth:'Promociones y cupones',close:'Cierre operativo diario',audit:'Auditoría',merchants:'Negocios',couriers:'Repartidores'}[tab]}</h1></div><div className="header-actions">{tab==='merchants'&&<button className="primary" onClick={()=>setShowMerchant(true)}>+ Crear negocio</button>}{tab==='couriers'&&<button className="primary" onClick={()=>setShowCourier(true)}>+ Registrar repartidor</button>}<button onClick={load}>↻ Actualizar</button></div></header>
  {msg&&<div className="alert">{msg}<button onClick={()=>setMsg('')}>×</button></div>}
  {tab==='home'&&<><section className="kpis"><article><small>Requieren atención</small><b>{central.filter(x=>x.attention!=='OK').length}</b></article><article><small>Incidencias abiertas</small><b>{incidents.filter(x=>x.status==='open').length}</b></article><article><small>Pedidos activos</small><b>{central.length}</b></article><article><small>Reembolsos</small><b>{refunds.filter(x=>x.status==='approved').length}</b></article></section><section className="panel table"><div className="head"><h2>Prioridad operativa</h2><span>Actualización en vivo</span></div><table><thead><tr><th>Pedido</th><th>Negocio</th><th>Estado</th><th>Tiempo</th><th>Atención</th><th/></tr></thead><tbody>{central.map(x=><tr className={x.attention!=='OK'?'central-alert-v40':''} key={x.order_id}><td>#{x.order_id.slice(0,8)}</td><td>{x.merchant_name}</td><td>{labels[x.status]||x.status}</td><td>{Math.round(x.age_minutes)} min</td><td><mark>{x.attention==='NEGOCIO_NO_RESPONDE'?'Negocio no responde':x.attention==='SIN_REPARTIDOR'?'Listo sin repartidor':x.attention==='INCIDENCIA'?`${x.open_incidents} incidencia(s)`:'En curso'}</mark></td><td><button onClick={()=>setSelected(orders.find(o=>o.id===x.order_id))}>Gestionar</button></td></tr>)}</tbody></table></section><section className="panel table"><div className="head"><h2>Incidencias abiertas</h2><span>{incidents.filter(i=>i.status==='open').length}</span></div><table><thead><tr><th>Pedido</th><th>Detalle</th><th>Fecha</th><th/></tr></thead><tbody>{incidents.filter(i=>i.status==='open').map(i=><tr key={i.id}><td>#{i.order_id.slice(0,8)}</td><td><b>{i.category}</b><small>{i.description}</small></td><td>{new Date(i.created_at).toLocaleString('es-MX')}</td><td><button onClick={()=>resolveIncident(i)}>Resolver</button></td></tr>)}</tbody></table></section></>}
  ...
  {tab==='reconciliation'&&<><div className="section-intro"><div><b>Clip vs Guti</b><span>Detecta pagos aprobados en Clip que no coincidan con el estado del pedido en Guti.</span></div><button onClick={refreshClipReconciliation} disabled={reconBusy}>{reconBusy?'Consultando Clip...':'Revisar con Clip'}</button></div><section className="panel table"><table><thead><tr><th>Pago Clip</th><th>Pedido</th><th>Monto</th><th>Clip</th><th>Guti pago</th><th>Pedido</th><th>Resultado</th></tr></thead><tbody>{reconciliation.map(r=><tr className={r.reconciliation_status!=='OK'?'recon-mismatch':''} key={r.payment_id}><td>{String(r.provider_payment_id||'').slice(0,16)}</td><td>{r.order_id?`#${r.order_id.slice(0,8)}`:'Sin pedido'}</td><td>{money(r.amount)}</td><td><b>{r.clip_last_status||r.guti_payment_record_status||'—'}</b></td><td>{r.guti_payment_record_status}</td><td>{r.order_payment_status||'—'}</td><td><mark>{r.reconciliation_status}</mark></td></tr>)}</tbody></table></section></>}
  {tab==='settlements'&&<>
  ...
  {tab==='growth'&&<><section className="panel coupon-builder-v38"><div className="head"><div><small>CRECIMIENTO</small><h2>Crear cupón</h2></div></div><form onSubmit={createCoupon} className="coupon-form-v38"><label>Código<input value={couponForm.code} onChange={e=>setCouponForm(p=>({...p,code:e.target.value.toUpperCase()}))} placeholder="GUTI50"/></label><label>Nombre<input value={couponForm.name} onChange={e=>setCouponForm(p=>({...p,name:e.target.value}))} placeholder="$50 de descuento"/></label><label>Tipo<select value={couponForm.discount_type} onChange={e=>setCouponForm(p=>({...p,discount_type:e.target.value}))}><option value="fixed">Monto fijo</option><option value="percent">Porcentaje</option><option value="free_delivery">Envío gratis</option></select></label><label>Valor<input type="number" value={couponForm.discount_value} onChange={e=>setCouponForm(p=>({...p,discount_value:e.target.value}))}/></label><label>Compra mínima<input type="number" value={couponForm.min_subtotal} onChange={e=>setCouponForm(p=>({...p,min_subtotal:e.target.value}))}/></label><label>Máx. descuento<input type="number" value={couponForm.max_discount} onChange={e=>setCouponForm(p=>({...p,max_discount:e.target.value}))} placeholder="Opcional"/></label><label>Negocio<select value={couponForm.merchant_id} onChange={e=>setCouponForm(p=>({...p,merchant_id:e.target.value}))}><option value="">Todos</option>{merchants.map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select></label><label>Límite por usuario<input type="number" value={couponForm.per_user_limit} onChange={e=>setCouponForm(p=>({...p,per_user_limit:e.target.value}))}/></label><label>Usos máximos<input type="number" value={couponForm.max_uses} onChange={e=>setCouponForm(p=>({...p,max_uses:e.target.value}))} placeholder="Sin límite"/></label><label>Inicio<input type="datetime-local" value={couponForm.starts_at} onChange={e=>setCouponForm(p=>({...p,starts_at:e.target.value}))}/></label><label>Fin<input type="datetime-local" value={couponForm.ends_at} onChange={e=>setCouponForm(p=>({...p,ends_at:e.target.value}))}/></label><label className="check-v38"><input type="checkbox" checked={couponForm.first_order_only} onChange={e=>setCouponForm(p=>({...p,first_order_only:e.target.checked}))}/>Solo primer pedido</label><button>Crear cupón</button></form></section><section className="panel table"><div className="head"><h2>Cupones</h2><span>{coupons.filter(c=>c.is_active).length} activos</span></div><table><thead><tr><th>Código</th><th>Beneficio</th><th>Negocio</th><th>Compra mín.</th><th>Estado</th><th/></tr></thead><tbody>{coupons.map(c=><tr key={c.id}><td><b>{c.code}</b><small>{c.name}</small></td><td>{c.discount_type==='fixed'?money(c.discount_value):c.discount_type==='percent'?`${c.discount_value}%`:'Envío gratis'}</td><td>{c.merchants?.name||'Todos'}</td><td>{money(c.min_subtotal)}</td><td><mark>{c.is_active?'Activo':'Inactivo'}</mark></td><td><button onClick={()=>toggleCoupon(c)}>{c.is_active?'Pausar':'Activar'}</button></td></tr>)}</tbody></table></section></>}
  {tab==='close'&&<section className="panel table"><div className="head"><div><small>CIERRE OPERATIVO</small><h2>Resumen por día</h2></div></div><table><thead><tr><th>Día</th><th>Entregados</th><th>Cancelados</th><th>Venta</th><th>Ingreso Guti</th><th>Neto negocios</th><th>Efectivo cobrado</th></tr></thead><tbody>{daily.map(d=><tr key={d.business_date}><td><b>{d.business_date}</b></td><td>{d.delivered_orders||0}</td><td>{d.cancelled_orders||0}</td><td>{money(d.gmv)}</td><td><b>{money(d.guti_revenue)}</b></td><td>{money(d.merchant_net)}</td><td>{money(d.cash_collected)}</td></tr>)}</tbody></table></section>}
  {tab==='audit'&&<section className="panel table"><div className="head"><div><small>TRAZABILIDAD</small><h2>Movimientos recientes</h2></div></div><table><thead><tr><th>Fecha</th><th>Acción</th><th>Entidad</th><th>ID</th><th>Actor</th></tr></thead><tbody>{audit.map(a=><tr key={a.id}><td>{new Date(a.created_at).toLocaleString('es-MX')}</td><td><b>{a.action}</b></td><td>{a.entity_type}</td><td>{String(a.entity_id||'').slice(0,12)}</td><td>{a.actor_id?String(a.actor_id).slice(0,8):'Sistema'}</td></tr>)}</tbody></table></section>}
